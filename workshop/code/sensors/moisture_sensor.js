...
const moistureSensorComponentName = 'moistureSensor';
...
class MoistureSensor {
  ...
  getMaxMinReportObject() {
    return {
      ...
      avgTemp: this.cumulativeLevel / this.numberOfReadings,
      endTime: (new Date(Date.now())).toISOString(),
      startTime: this.startTime
    };
  }
  getmaxLevelValueadings() {
...
const commandNameGetMaxMinReport1 = moistureSensorComponentName + commandComponentCommandNameSeparator + 'getMaxMinReport';
...
const commandHandler = async (request, response) => {
  ...
  case commandNameGetMaxMinReport1: {
    await sendCommandResponse(request, response, 200, moistureSensor.getMaxMinReportObject());
    break;
  }
...
async function main() {
  ...
  try {
    ...
    try {
      ...
      const patchmoistureSensorInfo = helperCreateReportedPropertiesPatch({
        maxLevelSinceLastReboot: moistureSensor.getmaxLevelValueadings(),
      }, moistureSensorComponentName);
      ...
      updateComponentReportedProperties(resultTwin, patchmoistureSensorInfo, moistureSensorComponentName);
      updateComponentReportedProperties(resultTwin, patchDeviceInfo, deviceInfoComponentName);
      desiredPropertyPatchListener(resultTwin, [moistureSensorComponentName, deviceInfoComponentName]);
    } catch (err) {
